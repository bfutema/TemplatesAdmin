var RENDERER = {
    ...
    setParameters : function () {
        this.$window = $(window);
        this.$container = $('#hexagon-background');
        this.$canvas = $('<canvas />');
    ...
    selectHexagon : function (event) {
        if ($(event.target).parent().hasClass('hexagon-background') && !$(event.target).hasClass('login-container')){
...
HEXAGON.prototype = {
    ...
    draw : function (context, targets) {
        for (var i = 0; i < targets.length; i++) {
            var target = targets[i], fillLuminance, strokeLuminance;
            ...
            if (target.count < this.COUNT.MIN * 2) {
                strokeLuminance = this.LUMINANCE.MIN + (this.LUMINANCE.MAX - this.LUMINANCE.MIN) * Math.sin(Math.PI / 2 * target.count / this.COUNT.IN / 2);
            } else if (target.count < this.COUNT.MAX) {
    ...
    render : function (context) {
        ...
        for (var i = this.sourceIndices.length - 1; i >= 0; i--) {
            var indices = this.sourceIndices[i], index = indices.indices[this.renderer.getRandomValue(0, 3)];
